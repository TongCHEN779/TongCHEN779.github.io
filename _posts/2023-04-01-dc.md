...
2. Dataset Distillation with Convexified Implicit Gradients (Loo et al. 2023) [[link](https://arxiv.org/abs/2302.06755)]: Implicit differentiation,
...
1. Dataset Condensation with Gradient Matching (Zhao et al. 2020) [[link](https://arxiv.org/abs/2006.05929)]: Gradient matching,
...
2. Loss-Curvature Matching for Dataset Selection and Condensation (Shin et al. 2023) [[link](https://arxiv.org/abs/2303.04449)]: Gradient matching + regularization,
...
1. Dataset Distillation by Matching Training Trajectories (Cazenavette et al. 2022) [[link](https://arxiv.org/abs/2203.11932)]: Trajectory matching,
...
1. Dataset Distillation with Infinitely Wide Convolutional Networks (Nguyen et al. 2021) [[link](https://arxiv.org/abs/2107.13034)]: Regression only,
...
2. Dataset Meat-Learning from Kernel Ridge-Regression (Nguyen et al. 2020) [[link](https://arxiv.org/abs/2011.00050)]: Regression only,

$$\min_{\mathcal{S}} \; d(\mathcal{T}_y, \theta_{\mathcal{S}} (\mathcal{T}_x)), \; \theta_{\mathcal{S}} = k(\cdot, \mathcal{S}_x) \big(k(\mathcal{S}_x, \mathcal{S}_x) + \lambda \mathbf{I}\big)^{-1} \mathcal{S}_y, \; k = \text{NTK}$$

3. CAFE: Learning to Condense Dataset by Aligning Features (Wang et al. 2022) [[link](https://arxiv.org/abs/2203.01531)]: Intermediate feature matching + output feature matching + regularization,
...
1. Dataset Condensation with Distribution Matching (Zhao et al. 2021) [[link](https://arxiv.org/abs/2110.04181)]: Integral probability metric (IPM),
...
2. M3D: Dataset Condensation by Minimizing Maximum Mean Discrepancy (Zhang et al. 2023) [[link](https://arxiv.org/abs/2312.15927)]: MMD,
...
3. Dataset Distillation via the Wasserstein Metric (Liu et al. 2024) [[link](https://arxiv.org/abs/2311.18531)]: Wasserstein distance,
...
4. Dataset Condensation by Minimal Finite Covering (Chen et al. 2024) [[link]()]: Hausdorff distance, average Hausdorff distance,
...
2. DiM: Distilling Dataset into Generative Model (Wang et al. 2023) [[link](https://arxiv.org/abs/2303.04707)]: Distribution matching + GAN training,
...
3. Dataset Condensation via Generative Model (Zhang et al. 2023) [[link](https://arxiv.org/abs/2309.07698)]: Feature matching + GAN training + regularization,
...
5. Dataset Distillation via Factorization (Liu et al. 2022) [[link](https://arxiv.org/abs/2210.16774)]: DC + pre-trained hallucinator-extractor,
...
6. Efficient Dataset Distillation via Minimax Diffusion (Gu et al. 2024) [[link](https://arxiv.org/abs/2311.15529)]: Feature matching + diffusion training + regularization,
...
7. Dataset Condensation via Image Decomposition (Chen et al. 2024) [[link]()]: DC + low rank decomposition,

$$\min_{\mathcal{S}_r} \; L_{DC} (\mathcal{S}_r, \mathcal{T}), \; \mathcal{S}_r = \{\mathbf{XX}^T: \text{rank} (\mathbf{X}) = r\}$$

8. Dataset Condensation via Input Subspace Projection (Chen et al. 2024) [[link]()]: DC + data span,
...
3. Dataset Distillation using Neural Feature Regression (Zhou et al. 2022) [[link](https://arxiv.org/abs/2206.00719)]: Regression only,
...
1. Dataset Condensation with Contrastive Signals (Lee et al. 2022) [[link](https://arxiv.org/abs/2202.02916)]: Variant of gradient matching,

$$\mathbb{E}_{\theta_{1:T}} \big[d (\mathbb{E}_{c} [\nabla_{\theta} L_{\mathcal{S}} (\theta)], \mathbb{E}_{c} [\nabla_{\theta} L_{\mathcal{T}} (\theta)])\big]$$

2. Improved Distribution Matching for Dataset Condensation (Zhao et al. 2023) [[link](https://arxiv.org/abs/2307.09742)]: Distribution matching + multi formation + data augmentation + model enrichment,
...
3. DataDAM: Efficient Dataset Distillation with Attention Matching (Sajedi et al. 2023) [[link](https://arxiv.org/abs/2310.00093)]: Distribution Matching + spatial attention matching (SAM) + $\ell_2$-normalization,
...
1. DREAM: Efficient Dataset Distillation by Representative Matching (Liu et al. 2023) [[link](https://arxiv.org/abs/2302.14416)]: Gradient matching + K-means clustering,
...
Information (Chen et al. 2022) [[link](https://arxiv.org/abs/2211.04446)]: Gradient matching for private set generation.

1. Connect the dots: Dataset Condensation, Differential Privacy, and Adversarial Uncertainty (Kenneth Odoh 2024) [[link](https://arxiv.org/abs/2402.10423)]

1. DP-MERF: Differentially Private Mean Embeddings with Random Features for Practical Privacy-Preserving Data Generation (Harder et al. 2021) [[link](https://arxiv.org/abs/2002.11603)]: MMD for DP.

1. Privacy for Free: How does Dataset Condensation Help Privacy? (Dong et al. 2022) [[link](https://arxiv.org/abs/2206.00240)]

1. No Free Lunch in "Privacy for Free: How does Dataset Condensation Help Privacy" (Carlini et al. 2022) [[link](https://arxiv.org/abs/2209.14987)]

### Robustness
---
1. Robustness May Be at Odds with Accuracy (Tsipras et al. 2018) [[link](https://arxiv.org/abs/1805.12152)]

1. Adversarial Examples Are Not Bugs, They Are Features (Ilyas et al. 2019) [[link](https://arxiv.org/abs/1905.02175)]

1. Can we achieve robustness from data alone? (Tsilivis et al. 2022) [[link](https://arxiv.org/abs/2207.11727)]

1. Towards Robust Dataset Learning (Wu et al. 2022) [[link](https://arxiv.org/abs/2211.10752)]

1. Towards Adversarially Robust Dataset Distillation by Curvature Regularization (Xue et al. 2024) [[link](https://arxiv.org/abs/2403.10045)]

### Graph Neural Network
---
1. Graph Condensation for Graph Neural Networks (Jin et al. 2021) [[link](https://arxiv.org/abs/2110.07580)]: Gradient matching for GNN.

1. Condensing Graphs via One-Step Gradient Matching (Jin et al. 2022) [[link](https://arxiv.org/abs/2206.07746)]: Gradient matching for GNN.

1. Graph Condensation via Receptive Field Distribution Matching (Liu et al. 2022) [[link](https://arxiv.org/abs/2206.13697)]: Distribution matching for GNN.

1. Kernel Ridge Regression-Based Graph Dataset Distillation (Xu et al. 2023) [[link](https://dl.acm.org/doi/10.1145/3580305.3599398)]: KIP for GNN.

### Medical Image Analysis
---
1. Progressive trajectory matching for medical dataset distillation (Yu et al. 2024) [[link](https://arxiv.org/abs/2403.13469)]: Trajectory matching for MedMNIST.

Reviews
---

1. Dataset Distillation: A Comprehensive Review (Yu et al. 2023) [[link](https://arxiv.org/abs/2301.07014)]

2. A Survey on Dataset Distillation: Approaches, Applications and Future Directions (Geng et al. 2023) [[link](https://arxiv.org/abs/2305.01975)]

3. A Comprehensive Study on Dataset Distillation: Performance, Privacy, Robustness and Fairness (Chen et al. 2023) [[link](https://arxiv.org/abs/2305.03355)]

4. A Survey on Dataset Distillation: Approaches, Applications and Future Directions (Geng et al. 2023) [[link](https://arxiv.org/abs/2305.01975)]

5. Data Distillation: A Survey (Sachdeva et al. 2023) [[link](https://arxiv.org/abs/2301.04272)]

6. A comprehensive survey of dataset distillation (Lei et al. 2023) [[link](https://arxiv.org/abs/2301.05603)]

7. A review of dataset distillation for deep learning (Le et al. 2022) [[link](https://ieeexplore.ieee.org/abstract/document/9932086)]

Benchmarks
---

1. DC-BENCH: Dataset Condensation Benchmark (Cui et al.) [[link](https://dc-bench.github.io/)]

2. DD-RobustBench: An Adversarial Robustness Benchmark for Dataset Distillation (Wu et al.) [[link](https://arxiv.org/abs/2403.13322)]
