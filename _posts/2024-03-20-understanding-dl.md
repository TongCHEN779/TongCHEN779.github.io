...
Actually, &dollar;\log p (x) = ELBO (x) + \mathcal{D}_{KL} (\phi (\cdot|x) \|| p(\cdot|x))&dollar;, which means we must pay a price if we use an approximation of the posterior $p(\cdot|x)$. Futhermore, ELBO can be written as the difference between a reconstruction error and a prior matching error:
