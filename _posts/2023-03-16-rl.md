...
$$J (\theta) = \mathbb{E}_{S \sim d_{{\pi}_{\theta}}} [V_{\pi_{\theta}} (S)] = \mathbb{E}_{S \sim d_{{\pi}_{\theta}}} \big[\mathbb{E}_{A \sim \pi_{\theta} (\cdot \vert S)} [Q_{\pi_{\theta}} (S, A)]\big],$$

where $d_{{\pi}_{\theta}}$ is the stationary distribution of Markov chain for $\pi_{\theta}$, defined as:

$$d_{{\pi}_{\theta}} (s) = \lim_{t \rightarrow \infty} \mathbb{P} (S_t = s \vert S_0 = s_0).$$

The policy gradient theorem states that:

$$\nabla_{\theta} J (\theta) \propto \mathbb{E}_{S \sim d_{{\pi}_{\theta}}} \big[\mathbb{E}_{A \sim \pi_{\theta} (\cdot \vert S)} [Q_{\pi_{\theta}} (S, A) \nabla_{\theta} \ln \pi_{\theta} (A \vert S)]\big].$$