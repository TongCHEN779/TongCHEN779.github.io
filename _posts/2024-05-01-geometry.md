...
As we see, the ddiferential operator are defined as the canonical homomorphism $\text{d}_p: \mathcal{F}_p \to \mathcal{F}_p / \mathcal{H}_p$. The cotangent space is spanned by the natural basis $\lbrace \text{d}_p (x^i) \rbrace_{i = 1}^m$, and we have $\text{d}_p (f) = \partial_i f(p) \text{d}_p (x^i)$, where the partial derivative is computed via the local chart $\partial_i f(p) := \frac{\partial (f \circ \varphi_U^{-1})}{\partial x^i} \big\vert_{x^i (p)}$.
