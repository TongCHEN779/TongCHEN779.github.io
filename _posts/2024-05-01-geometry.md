...
As we see, the ddiferential operator are defined as the canonical homomorphism $d_p: \mathcal{F}_p \to \mathcal{F}_p / \mathcal{H}_p$. The cotangent space is spanned by the natural basis $\lbrace d_p (x^i) \rbrace$, and we have $d_p (f) = \partial_i f(p) d_p (x^i)$, 
