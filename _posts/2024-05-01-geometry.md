...
As we see, the ddiferential operator are defined as the canonical homomorphism $\text{d}_p: \mathcal{F}_p \to \mathcal{F}_p / \mathcal{H}_p$. The cotangent space is spanned by the natural basis $\lbrace \text{d}_p (x^i) \rbrace$, and we have 
...
Then the oriented derivative of $[f] \in \mathcal{F}_p$ along $\gamma \in \Gamma_p$ at point $p$ is defined as a functional over $\mathcal{F}_p$:
